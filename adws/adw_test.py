...
- GITHUB_PAT: (Optional) GitHub Personal Access Token - only if using a
  different account than 'gh auth login'
"""
...
from adw_modules.utils import parse_json, setup_logger
...
def log_test_results(
    ...
    summary += f"**Failed:** {failed_count} [FAIL]\n\n"

    if results:
        summary += "#### Details:\n"
        for result in results:
            status = "" if result.passed else "[FAIL]"
    ...
    if e2e_results:
        ...
        summary += f"**Failed:** {e2e_failed_count} [FAIL]\n\n"
...
def format_test_results_comment(
    ...
    if failed_tests:
        ...
        comment_parts.append("## ERROR: Failed Tests")
        comment_parts.append("")
    ...
    for idx, test in enumerate(failed_tests):
        ...
        make_issue_comment(
            ...
            format_issue_message(
                adw_id,
                agent_name,
                f"ERROR: Attempting to resolve: {test.test_name}\n```json\n{test_payload}\n```",
            ),
        )
        ...
        if response.success:
            ...
            make_issue_comment(
                ...
                format_issue_message(
                    adw_id,
                    agent_name,
                    f"ERROR: Failed to resolve: {test.test_name}",
                ),
            )
...
def run_tests_with_resolution(
    ...
    while attempt < max_attempts:
        ...
        if not test_response.success:
            ...
            make_issue_comment(
                ...
                format_issue_message(
                    adw_id,
                    AGENT_TESTER,
                    f"ERROR: Error running tests: {test_response.output}",
                ),
            )
            break
        ...
        make_issue_comment(
            ...
            format_issue_message(
                adw_id,
                "ops",
                f"ERROR: Found {failed_count} failed tests. Attempting resolution...",
            ),
        )
    ...
    if attempt == max_attempts and failed_count > 0:
        logger.warning(
            f"Reached maximum retry attempts ({max_attempts}) with "
            f"{failed_count} failures remaining"
        )
        make_issue_comment(
            issue_number,
            format_issue_message(
                adw_id,
                "ops",
                f"[WARNING] Reached maximum retry attempts ({max_attempts}) with "
...
def execute_single_e2e_test(
    ...
    try:
        ...
        status_emoji = "" if e2e_result.passed else "[FAIL]"
        make_issue_comment(
            issue_number,
            format_issue_message(
                adw_id,
                ...
                f"{e2e_result.model_dump_json(indent=2)}\n```",
            ),
        )
        ...
        make_issue_comment(
            ...
            format_issue_message(
                adw_id,
                agent_name,
                f"ERROR: E2E test completed: {test_name}\n```json\n"
                f"{e2e_result.model_dump_json(indent=2)}\n```",
            ),
        )
...
def format_e2e_test_results_comment(
    ...
    if not results:
        return "ERROR: No E2E test results found"
    ...
    if failed_tests:
        comment_parts.append("")
        comment_parts.append("## ERROR: Failed E2E Tests")
        comment_parts.append("")
        ...
        if response.success:
            ...
            make_issue_comment(
                ...
                format_issue_message(
                    adw_id,
                    agent_name,
                    f"ERROR: Failed to resolve E2E test: {test.test_name}",
                ),
            )
...
def run_e2e_tests_with_resolution(
    ...
    if attempt == max_attempts and failed_count > 0:
        logger.warning(
            f"Reached maximum E2E retry attempts ({max_attempts}) with "
            f"{failed_count} failures remaining"
        )
        make_issue_comment(
            issue_number,
            format_issue_message(
                adw_id,
                "ops",
                f"[WARNING] Reached maximum E2E retry attempts ({max_attempts}) with "
                f"{failed_count} failures",
            ),
        )
...
def main():
    ...
    if branch_name:
        ...
        if result.returncode != 0:
            ...
            make_issue_comment(
                issue_number,
                format_issue_message(
                    adw_id, "ops", f"ERROR: Failed to checkout branch {branch_name}"
                ),
            )
            sys.exit(1)
        ...
        if not success:
            ...
            make_issue_comment(
                issue_number,
                format_issue_message(adw_id, "ops", f"ERROR: Error creating branch: {error}"),
            )
            sys.exit(1)
        ...
        make_issue_comment(
            issue_number, format_issue_message(adw_id, "ops", f"Created test branch: {branch_name}")
        )

    make_issue_comment(issue_number, format_issue_message(adw_id, "ops", "Starting test suite"))
    ...
    make_issue_comment(
        issue_number,
        format_issue_message(adw_id, AGENT_TESTER, f"📊 Final test results:\n{results_comment}"),
    )
    ...
    if failed_count > 0:
        ...
        make_issue_comment(
            issue_number,
            format_issue_message(
                adw_id, "ops", "[WARNING] Skipping E2E tests due to unit test failures"
            ),
        )
        e2e_results = []
        e2e_passed_count = 0
        ...
        make_issue_comment(
            issue_number,
            format_issue_message(
                adw_id, "ops", "[WARNING] Skipping E2E tests as requested via --skip-e2e flag"
            ),
        )
        e2e_results = []
        e2e_passed_count = 0
    ...
    if error:
        ...
        make_issue_comment(
            issue_number,
            format_issue_message(
                adw_id, AGENT_TESTER, f"ERROR: Error committing test results: {error}"
            ),
        )
    ...
    if total_failures > 0:
        ...
        failure_msg = "ERROR: Test suite completed with failures:\n"
        if failed_count > 0:
