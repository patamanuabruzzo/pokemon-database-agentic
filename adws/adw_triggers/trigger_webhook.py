...
AVAILABLE_WORKFLOWS = ["adw_plan", "adw_build", "adw_test", "adw_plan_build", "adw_plan_build_test"]
...
async def github_webhook(request: Request):
    ...
    try:
        ...
        if workflow == "adw_build" and not provided_adw_id:
            print("adw_build requires an adw_id, skipping")
            workflow = None
        ...
        if workflow:
            ...
            try:
                make_issue_comment(
                    str(issue_number),
                    f"{ADW_BOT_IDENTIFIER} 🤖 ADW Webhook: "
                    f"Detected `{workflow}` workflow request\n\n"
            ...
            print(
                f"Ignoring webhook: event={event_type}, action={action}, "
