...
def main():
    ...
    if error:
        ...
        make_issue_comment(
            issue_number,
            format_issue_message(adw_id, "ops", f"ERROR: Error classifying issue: {error}"),
        )
        sys.exit(1)
    ...
    if error:
        ...
        make_issue_comment(
            issue_number,
            format_issue_message(adw_id, "ops", f"ERROR: Error generating branch name: {error}"),
        )
        sys.exit(1)
    ...
    if not plan_response.success:
        ...
        error_msg = f"ERROR: Error building plan: {plan_response.output}"
        make_issue_comment(
            issue_number,
            format_issue_message(adw_id, AGENT_PLANNER, error_msg),
        )
        sys.exit(1)
    ...
    plan_file_path, error = get_plan_file(plan_response.output, issue_number, adw_id, logger)

    if error:
        logger.error(f"Error finding plan file: {error}")
        error_msg = f"ERROR: Error finding plan file: {error}"
        make_issue_comment(
            issue_number,
            format_issue_message(adw_id, "ops", error_msg),
        )
        sys.exit(1)
    ...
    commit_msg, error = create_commit(AGENT_PLANNER, issue, issue_command, adw_id, logger)
    ...
    if error:
        ...
        make_issue_comment(
            issue_number,
            format_issue_message(
                adw_id, AGENT_PLANNER, f"ERROR: Error creating commit message: {error}"
            ),
        )
        sys.exit(1)
    ...
    if not success:
        ...
        make_issue_comment(
            issue_number,
            format_issue_message(adw_id, AGENT_PLANNER, f"ERROR: Error committing plan: {error}"),
        )
        sys.exit(1)

    logger.info(f"Committed plan: {commit_msg}")
    make_issue_comment(issue_number, format_issue_message(adw_id, AGENT_PLANNER, "Plan committed"))
