...
def classify_issue(
    ...
    minimal_issue_json = issue.model_dump_json(by_alias=True, include={"number", "title", "body"})
...
def build_plan(
    ...
    logger.debug(f"Building plan for issue #{issue.number} with command {command}")

    issue_plan_response = execute_template(issue_plan_template_request)

    logger.debug(f"Plan build response - success: {issue_plan_response.success}")
...
def implement_plan(plan_file: str, adw_id: str, logger: logging.Logger) -> AgentPromptResponse:
    ...
    template_json = implement_template_request.model_dump_json(indent=2, by_alias=True)
    logger.debug(f"implement_template_request: {template_json}")
...
def ensure_adw_id(
    issue_number: str, adw_id: Optional[str] = None, logger: Optional[logging.Logger] = None
) -> str:
