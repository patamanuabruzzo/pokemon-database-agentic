...
def get_claude_env() -> Dict[str, str]:
    ...
    if not github_pat:
        # Try to get token from gh CLI (may be stored in keyring)
        try:
            result = subprocess.run(
                ["gh", "auth", "token"], capture_output=True, text=True, encoding="utf-8", timeout=5
            )
            if result.returncode == 0 and result.stdout.strip():
                github_pat = result.stdout.strip()
        except (subprocess.TimeoutExpired, FileNotFoundError, Exception):
            pass  # gh not available or not authenticated

    if github_pat:
        required_env_vars["GITHUB_PAT"] = github_pat
        required_env_vars["GH_TOKEN"] = github_pat  # Claude Code uses GH_TOKEN

    # Filter out None values and explicitly exclude CLAUDECODE to prevent nested sessions
    filtered_env = {k: v for k, v in required_env_vars.items() if v is not None}

    # Ensure CLAUDECODE is not in the environment (should not be, but be explicit)
    filtered_env.pop("CLAUDECODE", None)
...
def prompt_claude_code(request: AgentPromptRequest) -> AgentPromptResponse:
    ...
    # Don't use -p flag (command line length limits), use stdin instead
    cmd = [CLAUDE_PATH]
    ...
    try:
        ...
        with open(request.output_file, "wb") as f:
            result = subprocess.run(
                cmd, input=prompt_bytes, stdout=f, stderr=subprocess.PIPE, env=env, cwd=project_root
            )

        # Decode stderr from bytes
            ...
            if result_message:
                ...
                if subtype == "error_during_execution":
                    error_msg = (
                        ...
                        "Agent encountered an error and did not return a result"
                    )
