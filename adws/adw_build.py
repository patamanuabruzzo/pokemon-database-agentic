...
def main():
    ...
    if state:
        ...
        state_json = json.dumps(state.data, indent=2)
        make_issue_comment(
            issue_number,
    ...
    if not state.get("branch_name"):
        ...
        logger.error(error_msg)
        make_issue_comment(issue_number, format_issue_message(adw_id, "ops", f"ERROR: {error_msg}"))
        sys.exit(1)
    ...
    if result.returncode != 0:
        ...
        make_issue_comment(
            issue_number,
            format_issue_message(adw_id, "ops", f"ERROR: Failed to checkout branch {branch_name}"),
        )
        sys.exit(1)
    ...
    make_issue_comment(
        issue_number, format_issue_message(adw_id, "ops", "Starting implementation phase")
    )

    # Implement the plan
    logger.info("Implementing solution")
    make_issue_comment(
        issue_number, format_issue_message(adw_id, AGENT_IMPLEMENTOR, "Implementing solution")
    )
    ...
    if not implement_response.success:
        ...
        error_msg = f"ERROR: Error implementing solution: {implement_response.output}"
    ...
    make_issue_comment(
        issue_number, format_issue_message(adw_id, AGENT_IMPLEMENTOR, "Solution implemented")
    )
    ...
    if error:
        ...
        error_msg = f"ERROR: Error creating commit message: {error}"
        make_issue_comment(issue_number, format_issue_message(adw_id, AGENT_IMPLEMENTOR, error_msg))
        sys.exit(1)
    ...
    if not success:
        ...
        error_msg = f"ERROR: Error committing implementation: {error}"
        make_issue_comment(issue_number, format_issue_message(adw_id, AGENT_IMPLEMENTOR, error_msg))
        sys.exit(1)
    ...
    make_issue_comment(
        issue_number, format_issue_message(adw_id, "ops", "Implementation phase completed")
    )
