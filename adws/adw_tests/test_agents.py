...
def main():
    ...
    with ThreadPoolExecutor(max_workers=len(MODELS)) as executor:
        ...
        for future in as_completed(future_to_model):
            ...
            try:
                ...
                all_success = False
                print(f"ERROR: {model} - Exception during parallel execution: {str(e)}")
    ...
    for model in MODELS:
        if model in results:
            ...
            status = "PASS" if success else "ERROR: FAIL"
            print(f"{status} - {message}")

    print(f"\nOverall: {'All tests passed!' if all_success else 'ERROR: Some tests failed!'}")
