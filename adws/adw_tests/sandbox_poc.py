...
with Sandbox(envs={"ANTHROPIC_API_KEY": api_key}) as sandbox:
    ...
    cmd1 = (
        "~/.npm-global/bin/claude -p 'What is 2+2?' --output-format text "
        "--dangerously-skip-permissions < /dev/null"
    )
    result = sandbox.commands.run(cmd1, timeout=30000)
    print("Q: What is 2+2?")
    ...
    print("\n--- Test 2: JSON output format ---")
    cmd2 = (
        "~/.npm-global/bin/claude -p 'What is the capital of France?' "
        "--output-format json --dangerously-skip-permissions < /dev/null"
    )
    result = sandbox.commands.run(cmd2, timeout=30000)
