...
def run_e2e_workflow_test(issue_number: str):
    ...
    # Log summary
    logger.info(
        f"E2E workflow test completed: {passed_count} passed, {failed_count} failed "
        f"out of {len(results)} total tests"
    )

    # Assertions
    assert isinstance(results, list), "Results should be a list"
    assert all(isinstance(r, E2ETestResult) for r in results), (
        "All results should be E2ETestResult objects"
    )
...
def main():
    ...
    try:
        ...
        if not os.getenv("ANTHROPIC_API_KEY"):
            print("[WARNING] Warning: ANTHROPIC_API_KEY not set, some functionality may fail")

        run_e2e_workflow_test(issue_number)
