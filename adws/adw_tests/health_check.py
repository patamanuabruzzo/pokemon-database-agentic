...
def check_env_vars() -> CheckResult:
    ...
    optional_vars = {
        "GITHUB_PAT": (
            "(Optional) GitHub Personal Access Token - only needed if you want "
            "ADW to use a different GitHub account than 'gh auth login'"
        ),
...
def check_claude_code() -> CheckResult:
    ...
    try:
        ...
        return CheckResult(
            ...
            error=(
                ...
                "Please install or set CLAUDE_CODE_PATH correctly."
            ),
        )
    ...
    try:
        ...
        if result.returncode != 0:
            return CheckResult(success=False, error=f"Claude Code test failed: {result.stderr}")
        ...
        return CheckResult(success=False, error="Claude Code test timed out after 30 seconds")
...
def run_health_check() -> HealthCheckResult:
    ...
    result = HealthCheckResult(success=True, timestamp=datetime.now().isoformat(), checks={})
    ...
    if not env_check.success:
        ...
        result.errors.extend([f"Missing required env var: {var}" for var in missing_required])
...
def main():
    ...
    # Print summary
    status_prefix = "" if result.success else "[FAIL]"
    status_text = "HEALTHY" if result.success else "UNHEALTHY"
    print(f"{status_prefix} Overall Status: {status_text}")
    ...
    for check_name, check_result in result.checks.items():
        status = "" if check_result.success else "[FAIL]"
        ...
        if check_result.error:
            print(f"   ERROR: Error: {check_result.error}")
        if check_result.warning:
            print(f"   [WARNING] Warning: {check_result.warning}")
    ...
    if not result.success:
        ...
        if any("disler" in w for w in result.warnings):
            print("   5. Fork/clone the repository and update git remote to your own repo")

    # If issue number provided, post comment
    if args.issue_number:
        print(f"\n📤 Posting health check results to issue #{args.issue_number}...")
        status_emoji = "" if result.success else "[FAIL]"
        comment = (
            f"{status_emoji} Health check completed: {'HEALTHY' if result.success else 'UNHEALTHY'}"
        )
        try:
